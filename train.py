    ...
    def validate(self, dataloader: DataLoader, criterion: nn.Module):
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader, desc="Validating"):
                ...
                political_loss = criterion(
                    results["political_logits"], political_labels
                )
                social_loss = criterion(results["social_logits"], social_labels)
                economic_loss = criterion(results["economic_logits"], economic_labels)
                foreign_loss = criterion(results["foreign_logits"], foreign_labels)
                ...
                total_loss_val = (
                    political_loss + social_loss + economic_loss + foreign_loss
                )
...
def load_data(
    ...
    political_labels = [
        political_label_map.get(label.lower(), 1) for label in df["political_lean"]
    ]

    social_label_map = {"liberal": 0, "moderate": 1, "conservative": 2}
    social_labels = [
        social_label_map.get(label.lower(), 1)
        for label in df.get("social_lean", ["moderate"] * len(texts))
    ]

    economic_label_map = {"left": 0, "center": 1, "right": 2}
    economic_labels = [
        economic_label_map.get(label.lower(), 1)
        for label in df.get("economic_lean", ["center"] * len(texts))
    ]

    foreign_label_map = {"isolationist": 0, "neutral": 1, "interventionist": 2}
    foreign_labels = [
        foreign_label_map.get(label.lower(), 1)
        for label in df.get("foreign_lean", ["neutral"] * len(texts))
    ]
...
def main():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="data/trained_model.pt",
        help="Path to save trained model",
    )
    parser.add_argument(
        "--create_sample", action="store_true", help="Create sample training data"
    )
    ...
    texts, political_labels, social_labels, economic_labels, foreign_labels = load_data(
        args.data_path
    )
    ...
    (
        ...
        test_size=0.2,
        random_state=42,
        stratify=political_labels,
    )

    train_dataset = PoliticalTextDataset(
        train_texts, train_pol, train_soc, train_econ, train_for
    )
