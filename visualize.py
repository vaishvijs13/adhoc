...
def apply_dimensionality_reduction(embeddings: np.ndarray, method: str = "umap", 
        ...
        with torch.no_grad():
            ...
            for j, text in enumerate(batch_texts):
                ...
                classifications.append({
                    ...
                    'political_confidence': float(torch.softmax(political_logits, dim=0)[political_pred]),
                    'social_confidence': float(torch.softmax(social_logits, dim=0)[social_pred]),
                    'economic_confidence': float(torch.softmax(economic_logits, dim=0)[economic_pred]),
                    'foreign_confidence': float(torch.softmax(foreign_logits, dim=0)[foreign_pred])
                })
...
async def visualize_ideology_space(payload: IdeologySpaceRequest):
    try:
        ...
        if ref_df is not None:
            ref_classifications = [{'political': pol} for pol in ref_df['political_lean']]
        else:
            ref_classifications = []
        ...
        if input_indices:
            fig.add_trace(go.Scatter(
                ...
                customdata=[all_classifications[i]['political'] for i in input_indices]
            ))
        ...
        if ref_indices:
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    size=8,
                    color=[colors[i] for i in ref_indices],
                    opacity=0.6
                ),
                text=[f"{politicians[i]}: {all_texts[i][:50]}..." for i in ref_indices],
                name='Reference Politicians',
                hovertemplate='<b>%{text}</b><br>Political: %{customdata}<extra></extra>',
                customdata=[all_classifications[i]['political'] for i in ref_indices]
            ))
        
        fig.update_layout(
            title="Political Ideology Space (UMAP Projection)",
        ...
        analysis = {
            ...
            'reference_points': len(ref_embeddings) if ref_embeddings is not None else 0,
            'ideology_distribution': {
                'left': sum(1 for c in all_classifications if c['political'] == 'left'),
                'center': sum(1 for c in all_classifications if c['political'] == 'center'),
                'right': sum(1 for c in all_classifications if c['political'] == 'right')
            }
        }
...
async def cluster_analysis(payload: ClusterAnalysisRequest):
    try:
        ...
        # Get classifications for each text
        classifications = classify_texts(texts)
        ...
        for cluster_id in range(payload.n_clusters):
            ...
            fig.add_trace(go.Scatter(
                ...
                text=[f"Cluster {cluster_id}: {texts[i][:50]}..." for i in cluster_indices],
                name=f'Cluster {cluster_id}',
                hovertemplate='<b>%{text}</b><br>Political: %{customdata}<extra></extra>',
                customdata=[classifications[i]['political'] for i in cluster_indices]
            ))
        
        # Add cluster centers
        centers_2d = np.array([reduced_embeddings[cluster_labels == i].mean(axis=0) 
        ...
        for cluster_id in range(payload.n_clusters):
            ...
            cluster_analysis[f"cluster_{cluster_id}"] = {
                ...
                'dominant_ideology': max(ideology_counts.items(), key=lambda x: x[1])[0] if ideology_counts else 'unknown',
                'sample_texts': cluster_texts[:3]  # Show first 3 texts as examples
            }
...
async def dimensional_analysis(payload: VisualizationRequest):
    try:
        ...
        # Get classifications for all dimensions
        classifications = classify_texts(texts)
        
        # Create multi-dimensional scatter plot
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Social vs Economic', 'Social vs Foreign', 
                          'Economic vs Foreign', 'Overall Political Distribution'),
        ...
        # Update layout
        fig.update_xaxes(title_text="Social Axis (Liberal ← → Conservative)", row=1, col=1)
        fig.update_yaxes(title_text="Economic Axis (Left ← → Right)", row=1, col=1)
        
        fig.update_xaxes(title_text="Social Axis (Liberal ← → Conservative)", row=1, col=2)
        fig.update_yaxes(title_text="Foreign Policy (Isolationist ← → Interventionist)", row=1, col=2)
        
        fig.update_xaxes(title_text="Economic Axis (Left ← → Right)", row=2, col=1)
        fig.update_yaxes(title_text="Foreign Policy (Isolationist ← → Interventionist)", row=2, col=1)
        ...
        analysis = {
            'ideology_distributions': {
                'social': {label: sum(1 for c in classifications if c['social'] == label) 
                          for label in ['liberal', 'moderate', 'conservative']},
                'economic': {label: sum(1 for c in classifications if c['economic'] == label) 
                           for label in ['left', 'center', 'right']},
                'foreign': {label: sum(1 for c in classifications if c['foreign'] == label) 
...
async def get_reference_data():
    try:
        ...
        stats = {
            ...
            'unique_politicians': ref_df['politician'].nunique() if 'politician' in ref_df.columns else 0,
            'ideology_distribution': ref_df['political_lean'].value_counts().to_dict() if 'political_lean' in ref_df.columns else {},
            'date_range': {
                'earliest': ref_df['date'].min() if 'date' in ref_df.columns else None,
                'latest': ref_df['date'].max() if 'date' in ref_df.columns else None
            } if 'date' in ref_df.columns else None,
            'politicians': ref_df['politician'].unique().tolist() if 'politician' in ref_df.columns else []
        }
