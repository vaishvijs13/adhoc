...
class EmbeddingUpdater:
    def __init__(self, embeddings_file="data/reference_embeddings.csv", 
    ...
    def add_embedding(self, text: str, politician: str, embedding: np.ndarray,
                     date: str = None, source: str = None, metadata: dict = None):
        try:
            if os.path.exists(self.embeddings_file):
                df = pd.read_csv(self.embeddings_file)
            else:
                df = pd.DataFrame(columns=['text', 'politician', 'date', 'source', 'metadata', 'embedding'])
    ...
    def get_stats(self):
        try:
            ...
            csv_size = os.path.getsize(self.embeddings_file) if os.path.exists(self.embeddings_file) else 0
            faiss_size = os.path.getsize(self.index_file) if os.path.exists(self.index_file) else 0
...
async def add_political_statement(request: EmbeddingUpdateRequest):
    try:
        ...
        if not success:
            raise HTTPException(status_code=500, detail="Failed to add embedding to database")
