...
class IdeologyEmbeddingModel(nn.Module):
    def __init__(self, model_name="bert-base-uncased", projection_dim=256, num_political_labels=3):
        super().__init__()
    ...
    def forward(self, input_texts: List[str], return_attention: bool = True):
        ...
        if return_attention:
            ...
            results['tokens'] = [self.tokenizer.convert_ids_to_tokens(ids) for ids in encoding['input_ids']]
            
        return results
    
    def get_token_importance(self, text: str, top_k: int = 10) -> List[Tuple[str, float]]:
        with torch.no_grad():
            results = self.forward([text], return_attention=True)
            attention = results['attention_weights'][0].mean(dim=0)  # Average across heads
