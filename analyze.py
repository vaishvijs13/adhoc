    ...
    def create_sample_reference_data(self):
        sample_data = {
            'politician': [
                ...
                'Joe Biden', 'Kamala Harris', 'Chuck Schumer',
                'Donald Trump', 'Ron DeSantis', 'Ted Cruz',
                'Mitt Romney', 'Susan Collins', 'John McCain'
            ],
            'text': [
            ...
            'date': [
                ...
                '2023-01-25', '2023-02-15', '2023-03-05',
                '2023-01-30', '2023-02-25', '2023-03-15',
                '2023-01-20', '2023-02-10', '2023-03-01'
            ]
        }
        ...
        for i in range(k):
            if i < len(indices[0]):
                ...
                results.append({
                    ...
                    'sample_text': row['text'][:200] + "..." if len(row['text']) > 200 else row['text'],
...
async def analyze_text(payload: AnalyzeRequest):
    try:
        ...
        political_label, political_confidence = classify_ideology_scores(political_logits, 'political')
        social_label, social_confidence = classify_ideology_scores(social_logits, 'social')
        economic_label, economic_confidence = classify_ideology_scores(economic_logits, 'economic')
        foreign_label, foreign_confidence = classify_ideology_scores(foreign_logits, 'foreign')
...
async def health_check():
    try:
        ...
        return {
            ...
            "reference_db_size": len(reference_db.df) if reference_db.df is not None else 0
        }
    except Exception as e:
